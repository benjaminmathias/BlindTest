...
export function normalizeWithIndex(text: string): { normalized: string; map: number[] } {
  ...
  for (let index = 0; index < text.length; index += 1) {
    const base = (text[index] ?? '').normalize('NFD').replace(/[\u0300-\u036f]/g, '').toLowerCase()
...
const words = (value: string): string[] =>
  normalizeComparableText(value).split(/[^a-z0-9]+/).filter(Boolean)
...
export const isSameSong = (first: SongIdentity, second: SongIdentity): boolean =>
  getCanonicalSongKey(first) === getCanonicalSongKey(second)
